...
export default function PathwaySuggestions({
    stages, onIncorporate, focusStageId, onFocusClear,} : {stages:Stage[]; onIncorporate: (stage: Stage) => Promise<void>; focusStageId?: string | null; onFocusClear?: () => void;} ) {
    ...
    async function handleIncorporate(s: Suggestion, i: number) {
        if (!s.stage) return;
        const newStage: Stage = {
            ...s.stage, id: `ai-${Date.now()}-${i}`,
        };
        setIncorporatingIndex(i);
        await onIncorporate(newStage);
        ...
        <div className="border border-gray-200 rounded-xl p-5 w-full md:w-80 shrink-0">
            ...
            <div className="flex flex-col gap-3 mt-4">
                {suggestions?.map((s, i) => {
                    ...
                    const busy = incorporatingIndex === i;
                    return (
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <button onClick={() => {handleIncorporate(s, i);}}
                                        ...
                                        { done? "Added":busy ? "Adding..." : "Add to pathway"}
                                    </button>
                                    )}
                                </div>
                            </div>
